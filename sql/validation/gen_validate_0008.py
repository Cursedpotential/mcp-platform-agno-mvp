"""Generate the transaction-rollback validation harness for sql/0008_working_schema.sql.
...
Run:  uv run --no-sync python sql/validation/gen_validate_0008.py
Then: ssh <db-host> "docker exec -i <pg-container> psql -U ai -d ai" \
        < sql/validation/validate_0008_working_schema.sql
...
MIGRATION = REPO / "sql" / "0008_working_schema.sql"
OUT = REPO / "sql" / "validation" / "validate_0008_working_schema.sql"

PROBES = r"""
-- ===========================================================================
-- VALIDATION HARNESS - generated, do not hand-edit.
-- Everything above this line is sql/0008_working_schema.sql verbatim, minus
...
VALUES (:'run_id','evidence.raw_sms','1','person','Probe Person','probe person',0.9,'deadbeef')
...
VALUES (:'run_id','evidence.raw_sms','1','contact','probe event', now(),
        tstzrange(now() - interval '1 day', now(), '[)'), 'beef01', :'ent_id');
...
VALUES (:'run_id','evidence.raw_sms','2','contact','no time given','cafe02');
...
VALUES (:'run_id','evidence.raw_sms','1','person','Probe Person','probe person','deadbeef');
...
\echo '=== POST-ROLLBACK: working schema must be GONE ==='
SELECT count(*) AS working_schema_rows
  FROM information_schema.schemata WHERE schema_name='working';
"""
