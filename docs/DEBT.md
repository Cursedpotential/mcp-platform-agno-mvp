...
| Workbench canonical source/chunk visibility | **Resolved for the Swift operator loop:** `/intake` submits to neutral `/v1/ingest`; `/knowledge` source browse/detail now uses neutral `/v1/knowledge/items`, not AgentOS's content catalog. The authenticated operator can inspect parser id, versioned chunker id, source SHA-256, every normalized record/chunk, timestamps, disclosure fields, custody references, and raw attrs. Projection search is separately labeled Weaviate supplemental recall. Full Workbench API: 109 passed; ESLint and Next production build pass. The scratch PostgreSQL proof already observed Markdown and SBV rows through the same canonical read model. | **Data Explorer consolidation remains:** connect the existing governed record-metadata editor and PG schema view into the source drill-down; replace the stale Milvus schema pane with read-only Weaviate collection/vector metadata, dimensions, model/version, filters, and safe row samples. PostgreSQL stays canonical; projection rows must remain explicitly labeled. Live Workbench deployment remains owner-gated. |
