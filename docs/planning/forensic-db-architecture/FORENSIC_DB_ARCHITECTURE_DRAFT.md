...
> 2. **Semantica** (the user's Semantica / Hawksight / HawkSite knowledge-graph application) — knowledge-graph construction, context graphs, explainable reasoning, provenance & governance, graph analysis/traversal. **Semantica runs seed-first hybrid and writes OUR Neo4j + Milvus** (ADR ~0035 [citation corrected 2026-07-11: no Semantica ADR exists — the real ADR-0035 is the tools sub-namespacing decision. Semantica's actual integration design lives in `server/analysis/semantica_wiring.py` (design-only, approvals-gated), which specifies Neo4j role `read_derive` ONLY (Graphiti remains the sole Neo4j writer, ADR-0014) — NOTE this contradicts this draft's 'writes OUR Neo4j' phrasing; resolving that is Topic 6 of docs/reference/agno-memory-and-storage/07-platform-mapping.md §D]).
>
...
Semantica is the **case knowledge-graph builder** and runs **seed-first hybrid** (ADR ~0035 — phantom citation, see correction note above), writing **our Neo4j + Milvus**. Two phases:
...
2. **Semantica implementation is assumed, not confirmed (ADR ~0035 is a PHANTOM — no such ADR exists; see correction note above).** The seed-first-hybrid + writes-our-Neo4j+Milvus model is taken from the Context Pack and the task brief; the adapter layer (§8) is the deliberate hedge so a different real Semantica wiring changes only the adapter binding. **Resolved 2026-07-11: there are no ADR-0035 Semantica specifics to confirm — the citation was phantom. The authoritative wiring is `server/analysis/semantica_wiring.py` (read_derive, no writes).**
