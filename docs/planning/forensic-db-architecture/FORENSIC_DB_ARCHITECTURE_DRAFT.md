...
> 2. **Semantica** (the user's Semantica / Hawksight / HawkSite knowledge-graph application) — knowledge-graph construction, context graphs, explainable reasoning, provenance & governance, graph analysis/traversal. **Semantica runs seed-first hybrid and writes OUR Neo4j + Milvus** (ADR ~~~0035~~ **-> ADR-0043** [citation corrected 2026-08-05: the `~0035` citation was a phantom -- the real ADR-0035 is the tools sub-namespacing decision. The Semantica ADR now EXISTS: **ADR-0043, Semantica as a governed extraction worker (pinned fork); SurrealDB exits the critical path** (accepted 2026-08-02). Note ADR-0043 and `server/analysis/semantica_wiring.py` both give Semantica Neo4j role `read_derive` ONLY -- Graphiti remains the sole Neo4j writer (ADR-0014), which CONTRADICTS this draft's 'writes OUR Neo4j' phrasing. The ADR wins; this draft is superseded on that point]).
>
...
Semantica is the **case knowledge-graph builder** and runs **seed-first hybrid** (ADR ~~~0035~~ **-> ADR-0043** [citation corrected 2026-08-05: the `~0035` citation was a phantom -- the real ADR-0035 is the tools sub-namespacing decision. The Semantica ADR now EXISTS: **ADR-0043, Semantica as a governed extraction worker (pinned fork); SurrealDB exits the critical path** (accepted 2026-08-02). Note ADR-0043 and `server/analysis/semantica_wiring.py` both give Semantica Neo4j role `read_derive` ONLY -- Graphiti remains the sole Neo4j writer (ADR-0014), which CONTRADICTS this draft's 'writes OUR Neo4j' phrasing. The ADR wins; this draft is superseded on that point]), writing **our Neo4j + Milvus**. Two phases:
