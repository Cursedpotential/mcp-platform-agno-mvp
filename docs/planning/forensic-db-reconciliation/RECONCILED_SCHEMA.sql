-- =====================================================================================
-- RECONCILED_SCHEMA.sql  —  Forensic-Evidence DB, consolidated reconciliation (DRAFT)
-- Byline: Claude Code · Opus 4.8 (1M) · 2026-06-30
-- =====================================================================================
--
--  ####################################################################################
--  #  DRAFT — PAPER-ONLY. NOT YET DIFFED/APPLIED AGAINST THE LIVE DB.                  #
--  #  This file is assembled from the per-domain reconciliations (D1–D8) + the store  #
--  #  docs (S1 Milvus / S2 Neo4j / S3 SurrealDB) with the cross-domain integrity and  #
--  #  court-safety review fixes applied. It has been reconciled AGAINST the captured   #
--  #  live introspection (live-introspection/) but NOT executed. Before ANY deploy:    #
--  #    1. psql \d / \dn / \dx / \dT against the running agno-postgres:18-duckdb       #
--  #    2. Milvus list_collections ; Neo4j db.schema ; Surreal INFO FOR DB             #
--  #    3. Apply via an idempotent migration (NOT this monolith) on the live volume.   #
--  #  Search this file for 'TODO(human):' for every decision that needs sign-off.      #
--  ####################################################################################
--
--  TARGET: unified PG18 resource `agno-postgres:18-duckdb` (DB `ai`, role `ai`),
--          Resource #1 of the 4-resource data tier
--          (PG+PostGIS+embedded DuckDB(pg_duckdb) · Milvus · Neo4j · SurrealDB).
--          PostGIS + DuckDB are EMBEDDED in this one resource; never standalone.
--
--  SECURITY BOUNDARY (as-built law — do not invent parallel top-level schemas):
--     evidence = raw/source + custody hashes. Agents READ-ONLY (connection-enforced via
--                default_transaction_read_only). Written only by the ingestion service role.
--     analysis = derived/normalized artifacts. Writes only after a recorded HITL approval.
--     public   = HITL audit + Agno-managed operational memory/registries.
--     (Live note: Agno's own tables actually live in schema `ai`, not `public`; D8
--      operational-memory tables intentionally land in `public`.)
--
--  EXECUTION CAUTION: this file contains `ALTER TYPE ... ADD VALUE` and `CREATE TYPE`
--  statements that CANNOT run inside a single transaction, and an ADD VALUE cannot be
--  USED in the same txn that adds it. Run STEP 1 (type adaptations) as standalone
--  statements / its own migration BEFORE the table DDL. Do NOT wrap the whole file in one
--  BEGIN/COMMIT.
-- =====================================================================================


-- =====================================================================================
-- STEP 0 — EXTENSION PREAMBLE  (truth = docker/postgres/Dockerfile + sql/0001 + live drift)
-- Live introspection (2026-06-30) shows citext/ltree/hstore/fuzzystrmatch are in sql/0001
-- but NOT installed on the live volume — this preamble re-asserts the full required set.
-- =====================================================================================
CREATE EXTENSION IF NOT EXISTS pgcrypto;        -- SHA-256 custody (digest/hmac); NOT for UUIDs
CREATE EXTENSION IF NOT EXISTS pg_trgm;         -- fuzzy/trigram (dedup, entity resolution)
CREATE EXTENSION IF NOT EXISTS fuzzystrmatch;   -- soundex/levenshtein/dmetaphone (id resolution)  [LIVE-DRIFT: install]
CREATE EXTENSION IF NOT EXISTS citext;          -- case-insensitive text (names/emails/handles)     [LIVE-DRIFT: install]
CREATE EXTENSION IF NOT EXISTS ltree;           -- hierarchical labels (decomposition path, MCL)     [LIVE-DRIFT: install]
CREATE EXTENSION IF NOT EXISTS hstore;          -- key-value attribute bags                          [LIVE-DRIFT: install]
CREATE EXTENSION IF NOT EXISTS btree_gin;       -- mixed scalar+text/jsonb composite indexes
CREATE EXTENSION IF NOT EXISTS btree_gist;      -- EXCLUDE on tstzrange (bitemporal no-overlap)
CREATE EXTENSION IF NOT EXISTS unaccent;        -- accent-insensitive FTS
CREATE EXTENSION IF NOT EXISTS vector;          -- pgvector — LEGACY/migration-only; vectors → Milvus (ADR-0027)
...
-- BM25: pg_textsearch is STAGED-not-baked (no PGDG package). Milvus owns primary
-- semantic/hybrid/BM25 (ADR-0027); PG keeps tsvector + pg_trgm for cheap local lookups.
-- Native PG18 uuidv7() is REQUIRED (ungated). Confirm SELECT version() >= 18 before deploy.


-- =====================================================================================
-- STEP 1 — SCHEMAS + CUSTOM TYPES  (reuse sql/0004; create only NEW types here)
-- =====================================================================================
...
-- ----------------------------------------------------------------------------
-- 1.0  Apply sql/0004 first if absent (live drift: 0004 NOT applied).
--      0004 ships: confidence, geo_point, canonical_id (domains);
--      entity_type, event_type, temporal_class, mcl_factor, source_system,
--      match_method, disclosure_tier (enums); source_ref (composite).
--      ==> Run `psql -f sql/0004_custom_types.sql` BEFORE this file on the live volume.
-- ----------------------------------------------------------------------------

-- 1.1  disclosure_tier double-definition FIX (E1 §5.1) — rename the orphan 0004 ENUM
--      (public|restricted|sealed = access classification) to sensitivity_tier so it stops
--      colliding with the substantive bitemporal column on analysis.normalized_record
--      (0003 TEXT CHECK contemporaneous|hindsight|discovered — the survivor, untouched).
--      Idempotent; safe to re-run.
DO $$ BEGIN
    ALTER TYPE disclosure_tier RENAME TO sensitivity_tier;
EXCEPTION
    WHEN undefined_object THEN NULL;   -- 0004 never applied, or already renamed
    WHEN duplicate_object THEN NULL;   -- sensitivity_tier already exists
END $$;
-- If 0004 was never applied on the live volume, create the access enum fresh:
DO $$ BEGIN
    CREATE TYPE sensitivity_tier AS ENUM ('public','restricted','sealed');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 1.2  NEW shared cross-domain enums (created ONCE; consolidates the per-domain
--      duplicates the integrity review flagged: A9 timestamp_certainty→precision_class,
--      A11 one party vocab, A9 single assertion_type enum).
DO $$ BEGIN CREATE TYPE evidence_tier AS ENUM
  ('raw','extracted','inferred','analytical','legal_conclusion');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;          -- the data_tier / processing-lane axis

DO $$ BEGIN CREATE TYPE assertion_type AS ENUM
  ('raw_evidence','extracted_fact','inferred_fact','analytical_finding','legal_conclusion');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;          -- the epistemic-class axis (1:1 with evidence_tier)

DO $$ BEGIN CREATE TYPE precision_class AS ENUM
  ('exact','approximate','inferred','uncertain');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;          -- canonical timestamp-certainty (was D3 timestamp_certainty)
...
-- ONE canonical party vocabulary (resolves A11's five competing sets). petitioner/respondent
-- map to user/partner via analysis.person.connection_to; this axis = "whose conduct".
...
-- Bitemporal knowledge-horizon enum for NEW tables (same vocabulary as the as-built
-- normalized_record.disclosure_tier TEXT column, which stays as-is — see TODO below).
DO $$ BEGIN CREATE TYPE disclosure_horizon AS ENUM
  ('contemporaneous','hindsight','discovered');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Domain-specific NEW enums:
...
--   stays AS-BUILT (TEXT CHECK contemporaneous|hindsight|discovered) — non-destructive.
--   The cross-store docs (S2/S3) call this concept `knowledge_horizon`. Decide ONCE whether
--   to (a) leave the column name `disclosure_tier`, or (b) rename it to `knowledge_horizon`
--   and retype to the disclosure_horizon enum (D3 §M2 — a coordinated records-domain
--   migration). Until decided, NEW tables use the disclosure_horizon enum with the SAME
--   vocabulary; the column is the canonical source. (integrity review A5)

-- 1.3  ADAPT (never redefine) the as-built 0004 entity_type + event_type via ADD VALUE.
--      !! RUN THESE OUTSIDE A TRANSACTION, BEFORE the table DDL — new enum values are not
--      usable in the txn that adds them. (D4 §2.0 / D3 §M3)
...
-- =====================================================================================
-- STEP 2 — SHARED GUARD FUNCTIONS (append-only / write-once / hash-chain)
-- =====================================================================================
...
-- =====================================================================================
-- STEP 3 — REFERENCE TABLES  (seeded; small, stable)
-- =====================================================================================

-- 3.1  MCL 722.23 (a)-(l) custody-factor reference (D7). PK = 0004 mcl_factor enum.
--      Statutory-canonical letters: J=facilitation, K=domestic_violence (D6 J<->K fix).
...
-- 4.1  Raw source registry (write-once original-of-record) — D1 §2.
CREATE TABLE IF NOT EXISTS evidence.source (
    id                  uuid PRIMARY KEY DEFAULT uuidv7(),
    sha256              bytea NOT NULL,                 -- H1 canonical identity (32B)
    md5_prefilter       bytea,                          -- pre-filter / CaseBible join ONLY (never identity)
    ...
    origin_device_id    uuid,                           -- SOFT ref into entity registry (no FK — RO lane never FKs analysis)
    origin_account      uuid,                           -- SOFT ref (no FK)
    ...
    -- MUTABLE WHITELIST: lifecycle status (authoritative history = custody_event)
    ...
    original_metadata   jsonb NOT NULL DEFAULT '{}',    -- as-received, never edited
    derived_metadata    jsonb NOT NULL DEFAULT '{}',    -- system-computed
...
-- 4.2  Recursive file decomposition (file->page->frame->screenshot->OCR->msg->event) — D1 §3.
CREATE TABLE IF NOT EXISTS evidence.file_node (
    ...
    mime_type       text,
    extraction_confidence confidence,                   -- 0004 domain
...
-- 4.3  EXTEND the as-built evidence.evidence_hash (D1 §4). As-built cols + 32B CHECK + digest
--      index are PRESERVED; new columns are purely additive.
...
-- 4.4  Chain-of-custody event log (append-only, pgcrypto sha256 hash-chained) — D1 §5.
...
-- 4.5  RAW GEO/TIMELINE (D5 is canonical; integrity A1 deletes D3's competing
--      raw_timeline_segment/timeline_waypoint). Custody anchor = evidence.source(id).
--      Order raw points by sequence/serial, NEVER by timestamp (E3 §D rule).
--      ingest_run_id is a SOFT uuid (no FK) — RO evidence lane never FKs into analysis (A7/A8).
CREATE TABLE IF NOT EXISTS evidence.gps_point (
    id             uuid PRIMARY KEY DEFAULT uuidv7(),
    source_id      uuid NOT NULL REFERENCES evidence.source(id),
    file_node_id   uuid REFERENCES evidence.file_node(id),
    device_id      uuid,                                -- SOFT ref to analysis.device (A7: no hard FK across the boundary)
    geog           geo_point NOT NULL,                  -- 0004 domain (PostGIS-guarded)
    captured_at    timestamptz,
    captured_raw   text,
    ts_precision   precision_class NOT NULL DEFAULT 'exact',
    accuracy_m     numeric(8,2),
    point_sequence bigint,
    ingest_run_id  uuid,                                -- SOFT ref -> analysis.processing_run
...
-- raw_path = pure raw waypoints. The multi-device split (an INFERENCE) is intentionally
-- NOT here — it lives in analysis.waypoint_device_split (integrity A1 lane fix).
CREATE TABLE IF NOT EXISTS evidence.raw_path (
    id                 uuid PRIMARY KEY DEFAULT uuidv7(),
    source_id          uuid NOT NULL REFERENCES evidence.source(id),
    file_node_id       uuid REFERENCES evidence.file_node(id),
    path_serial        bigint,
    point_sequence     bigint NOT NULL,                 -- 1-based; CRITICAL ordering
...
-- =====================================================================================
-- STEP 5 — PUBLIC: CONTEXT VERSION REGISTRIES + run anchor prerequisites (D8 §A)
-- =====================================================================================
...
-- =====================================================================================
-- STEP 6 — ANALYSIS: PROVENANCE / LINEAGE  (D8 §B). processing_run is THE single run
-- anchor (integrity A4/A8: drop analysis.provenance / evidence.ingestion_run names).
-- =====================================================================================
CREATE TABLE IF NOT EXISTS analysis.processing_run (
    ...
    input_evidence_ids uuid[]  NOT NULL DEFAULT '{}',   -- -> evidence.evidence_hash(id) (soft)
...
CREATE TABLE IF NOT EXISTS analysis.artifact_registry (
    ...
    artifact_kind     text NOT NULL,                    -- open vocab (see D8 note); court-safety is assertion_type, not this
...
-- =====================================================================================
-- STEP 7 — ANALYSIS: ENTITIES & IDENTITY RESOLUTION  (D4)
-- =====================================================================================
...
-- Communication/device identifiers — entities + bitemporal ownership. citext EXCLUDE keys
-- are cast to ::text so btree_gist has an opclass (integrity A14).
...
CREATE TABLE IF NOT EXISTS analysis.entity_mention (
    ...
    subject_type text, subject_id uuid,                 -- polymorphic soft ref
...
-- UNIFIED cross-store crosswalk (integrity A6: S1's pairwise shape + D4's optional entity anchor).
CREATE TABLE IF NOT EXISTS analysis.id_xref (
    id                  uuid PRIMARY KEY DEFAULT uuidv7(),
    canonical_entity_id uuid REFERENCES analysis.entity(id),   -- OPTIONAL (entity links); NULL for row<->vector links
    system_a            source_system NOT NULL, native_id_a text NOT NULL,
    system_b            source_system NOT NULL, native_id_b text NOT NULL,
    match_method        match_method NOT NULL,                 -- reuse 0004
    confidence          confidence,
    source              source_ref,                            -- reuse 0004 composite (provenance pointer)
...
-- append-only enforcement for evidence-grade rows
...
-- =====================================================================================
-- STEP 8 — ANALYSIS: NORMALIZED RECORD SPINE + MESSAGES  (D2)
-- =====================================================================================
-- Extend the as-built spine (0003). disclosure_tier (knowledge-horizon TEXT CHECK) UNCHANGED.
...
-- analysis.message — forensic subtype sharing the spine PK.
-- COURT-SAFETY (review #3): interpretive single-valued columns are *_hint and are NON-court-readable
-- (exclude from any agent court-read grant; authoritative labels live in relational_classification / pattern_finding).
CREATE TABLE IF NOT EXISTS analysis.message (
    ...
    conversation_id uuid NOT NULL REFERENCES analysis.conversation(id),   -- mirror of spine.conversation_ref
    ts_utc timestamptz,                                                    -- mirror of spine.occurred_at
    ...
    sender_raw text, sender_e164 text, sender_entity_id uuid,             -- soft -> entity (deferred FK)
...
-- Court-safe multi-label HITL surface. COURT-SAFETY (review #2): legal gate added.
...
-- =====================================================================================
-- STEP 9 — ANALYSIS: GEO / LOCATION  (D5). provenance_id is now a real nullable FK to
-- processing_run (integrity A4 fix: analysis.provenance never existed).
-- =====================================================================================
...
-- =====================================================================================
-- STEP 10 — ANALYSIS: EVENTS & BITEMPORAL TIMELINE  (D3). event_source_record now points
-- at D5's raw tables / normalized_record (integrity A1: D3 raw tables deleted).
-- =====================================================================================
...
CREATE TABLE IF NOT EXISTS analysis.event_source_record (
    ...
    source_id uuid REFERENCES evidence.source(id),       -- raw-segment anchor (D5 split: no single raw table)
    raw_ref jsonb,                                        -- {table, id} pointer into evidence.raw_visit/activity/path/trip
...
-- INFERRED multi-device split (lane-split out of raw per D3 rule; references D5 raw_path).
...
-- =====================================================================================
-- STEP 11 — ANALYSIS: SYNTHESIZED FINDING (NEW — integrity A3 requires this anchor)
-- =====================================================================================
...
CREATE TABLE IF NOT EXISTS analysis.finding (
    id uuid PRIMARY KEY DEFAULT uuidv7(),
    case_id uuid,
    finding_type text NOT NULL,                          -- behavioral_pattern|contradiction|timeline|location|claim|...
...
-- =====================================================================================
-- STEP 12 — ANALYSIS: BEHAVIORAL FINDINGS, PATTERNS & DETECTION CONFIG  (D6)
-- =====================================================================================
...
-- Per-match detection rows = HYPOTHESES. COURT-SAFETY: legal gate + attribution gate (#4) +
-- bias_caution/authored_perspective denormalized onto the finding row (#6).
CREATE TABLE IF NOT EXISTS analysis.pattern_finding (
    ...
    bias_caution boolean NOT NULL DEFAULT true, authored_perspective text,   -- review #6: travels with the finding
    ...
    -- review #4: attribution mandatory before legal use (inbound vs outbound means the opposite)
...
-- =====================================================================================
-- STEP 13 — ANALYSIS: LEGAL / CUSTODY, COURT-EVIDENCE SPINE, TASKS, EXPORT  (D7)
-- review_status columns use the shared review_state enum (integrity A11).
-- =====================================================================================
...
CREATE TABLE IF NOT EXISTS analysis.evidence_item (
    ...
    -- review #5: confidence_tier cannot claim 'high' on a low numeric confidence
...
-- factor_citation — court-safety #5: added safe_for_legal_use + gate.
...
-- legal_timeline_event — court-safety #5: added safe_for_legal_use + gate.
...
-- task triggers (snapshot + status log) + history immutability
...
-- =====================================================================================
-- STEP 14 — ANALYSIS: REVIEW / HITL + REDACTION / EXPORT  (D8 §D/§E)
-- =====================================================================================
...
-- =====================================================================================
-- STEP 15 — PUBLIC: WORK-PRODUCT OPERATIONAL MEMORY + AUDIT SPINE  (D8 §F/§G)
-- =====================================================================================
...
-- THE single append-only, hash-chained audit spine (merge of provenance.audit_log + change_log).
...
-- Timeline event evidence package (D3): current belief only, confidence-tiered, HITL-gated.
...
-- THE court-export view (D7): nothing leaves without ALL gates.
...
-- Cross-session "where was I" board (D7).
...
-- =====================================================================================
-- END RECONCILED_SCHEMA.sql (DRAFT) — verify against live before any deploy.
-- =====================================================================================
