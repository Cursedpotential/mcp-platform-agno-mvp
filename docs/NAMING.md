...
| **consignatio** | `consignatio` | The Vault / Case Bible system. Latin: the affixing of a seal; an attested document; written proof — the custody guarantee itself, not merely a storeroom. | D-141 (vacates and replaces the D-138 slot `vestigia`, which moved to geo under D-140) | **Product name only.** `casebible-*` R2 buckets, the `casebible` PG schema, the catalog skill, and `cb-*` commands are explicitly NOT renamed — see §3. |
...
| Case Bible / Vault (product name) | **consignatio** (product name only) | Product/brand name | D-141. `casebible-*` R2 buckets, `casebible` PG schema, the `cb-*` command family, and the catalog skill are **NOT renamed** — same reservation as D-138 item 2, restated in D-141. |
...
- `casebible-*` R2 buckets, the `casebible` PostgreSQL schema, the catalog
