...
- **Tool exposure pattern (owner, 2026-06-13):** heavy, long-running, or non-Python tools are wrapped as **FastAPI (or similar) HTTP services** — the existing `platform-tools` *tools-facade* pattern — then registered via the registry's HTTP runner and/or federated through **IBM ContextForge** (the MCP tool gateway). Lightweight Python tools stay in-process via `@register`. Either way the *contract* (capability + `NormalizedRecord`/payload I/O) is identical.
