"""contracts/records.py — the canonical record schema every parser emits into.
...
Home (ADR-0035, Option A): this is the platform's cross-domain record contract,
not an evidence-private type — imported by 15 parser modules + evidence
internals + tests + (by mount contract) the facade. It lives in the
import-light ``server.contracts`` package so the dep-light facade container can
import the parsers without dragging heavy deps in. ``server.evidence.normalize``
remains as a thin deprecated re-export shim for stragglers.
"""
