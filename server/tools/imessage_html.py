...
# specs/imessage-owner-format-PROVEN-prototype.py.
...
def _parse_owner_format(soup, conv_id: str) -> list[NormalizedRecord]:
    ...
    for seq, bubble in enumerate(soup.select("div.bubble")):
        cls = bubble.get_attribute_list("class")
    ...
    if not looks_like_imessage_html(soup):
        ...
        if looks_like_owner_imessage_html(soup):
            ...
            return records_out(
                ...
                calls=sum(1 for r in owner_records if r.record_type == RecordType.call),
                announcements=sum(1 for r in owner_records if r.record_type == RecordType.event),
                participants=len({r.role for r in owner_records if r.role}),
                variant="owner-custom",
            )
        raise ValueError(
