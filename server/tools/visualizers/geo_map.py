...
or by a user (CLI `python -m server.tools.visualizers.geo_map`). Input is a variable (list of
...
EXPECTED SCHEMA (the template; see server/tools/visualizers/geo_map_template.csv):
...
OUTPUTS
  out_html            self-contained Leaflet page (vendored JS/CSS from
                      server/tools/visualizers/assets/, CDN fallback if assets are missing)
...
def _leaflet_assets() -> tuple[str, str, list[str]]:
    ...
    warnings.append(
        "vendored Leaflet assets missing (server/tools/visualizers/assets/) — falling back to CDN; map needs internet"
    )
...
@register(
    ...
    provenance="ported from PR #15 (commit 667bd24); Leaflet 1.9.4 assets vendored upstream",
...
def _main() -> None:
    ...
    ap = argparse.ArgumentParser(
        prog="python -m server.tools.visualizers.geo_map",
