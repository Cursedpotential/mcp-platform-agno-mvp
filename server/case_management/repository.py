...
def get_case_management_capabilities() -> dict[str, object]:
    ...
    with _get_engine().connect() as conn:
        registry_available = bool(
            conn.execute(
                text(
                    "SELECT to_regclass('registry.matter') IS NOT NULL "
                    "AND to_regclass('registry.court_case') IS NOT NULL "
...
def list_matters(*, limit: int, offset: int) -> MatterList:
    with _get_engine().connect() as conn:
        total = int(conn.execute(text("SELECT count(*) FROM registry.matter")).scalar() or 0)
        rows = (
            conn.execute(
    ...
    try:
        with _get_engine().begin() as conn:
            row = (
                conn.execute(
                    text(
                        "INSERT INTO registry.matter (title, description, created_by) "
        ...
        cases = (
            conn.execute(
                text(
                    ...
                    "FROM registry.court_case WHERE matter_id = :matter_id "
...
def create_court_case(matter_id: UUID, body: CourtCaseCreate) -> CourtCase:
    try:
        with _get_engine().begin() as conn:
            if not conn.execute(text("SELECT 1 FROM registry.matter WHERE id = :id"), {"id": matter_id}).first():
                raise CaseRepositoryError("matter not found", 404)

            has_case = bool(
                conn.execute(
                    text("SELECT 1 FROM registry.court_case WHERE matter_id = :matter_id LIMIT 1"),
                    {"matter_id": matter_id},
                ).first()
            )
            ...
            if is_primary:
                conn.execute(
                    text("UPDATE registry.court_case SET is_primary = false WHERE matter_id = :matter_id"),
                    {"matter_id": matter_id},
                )
...
def _resolve_selected_source(conn: Any, matter_id: UUID, body: EvidenceItemCreate) -> dict[str, Any]:
    ...
    case_owner = conn.execute(
        text("SELECT 1 FROM registry.court_case WHERE id = :case_id AND matter_id = :matter_id"),
