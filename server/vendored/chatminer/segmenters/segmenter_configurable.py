...
Usage:
    from server.vendored.chatminer.segmenters import ConfigurableSegmenter
...
from server.vendored.chatminer.segmenters.segmenter_local import LocalSegmenter, SegmenterConfig
from server.vendored.chatminer.core.types import ParsedMessage
...
class ConfigurableSegmenter(LocalSegmenter):
    ...
    def __init__(
        ...
        # Provider selection
        self.provider = (provider if provider is not None else (os.getenv("SEGMENTER_PROVIDER") or "local")).lower()
        self.model_name = (
            model_name if model_name is not None else (os.getenv("SEGMENTER_MODEL") or self.config.model_name)
        )
