...
type HashBatchWriter interface {
	...
	Commit(context.Context, HashSummary) (resultRef proffer.Ref, receiptRef proffer.Ref, err error)
...
type HashRepository interface {
	...
	OpenRawRecords(context.Context, proffer.Ref) (ByteMemberStream, error)
	OpenNormalizedRecords(context.Context, proffer.Ref) (ByteMemberStream, error)
	OpenHashMembers(context.Context, proffer.Ref) (DigestMemberStream, error)
...
func (a HashActivities) FingerprintSource(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	return a.fingerprintSource(ctx, req, stagegraph.FingerprintSource)
}

func (a HashActivities) fingerprintSource(ctx context.Context, req proffer.StageRequest, stage stagegraph.StageID) (proffer.StageResult, error) {
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("open original %q: %w", originalRef, err)
	}
	defer reader.Close()
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("begin context source fingerprint batch: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("compute context source fingerprint: %w", err)
	}
	...
	if err := writer.Append(ctx, member); err != nil {
		return proffer.StageResult{}, fmt.Errorf("persist context source fingerprint member: %w", err)
	}
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("commit context source fingerprint: %w", err)
	}
	committed = true
...
func (a HashActivities) LegacyHashSource(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
...
func (a HashActivities) FingerprintRawRecords(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	if err := a.validate(); err != nil {
		return proffer.StageResult{}, err
	}
...
func (a HashActivities) LegacyHashRawRecords(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	return a.hashRecordBytes(ctx, req, recordHashConfig{
...
func (a HashActivities) HashNormalizedRecords(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	if err := a.validate(); err != nil {
		return proffer.StageResult{}, err
	}
	return a.hashRecordBytes(ctx, req, recordHashConfig{
...
type recordHashConfig struct {
	...
	open          func(context.Context, proffer.Ref) (ByteMemberStream, error)
	canon         func(ByteMember) (string, error)
}

func (a HashActivities) hashRecordBytes(ctx context.Context, req proffer.StageRequest, cfg recordHashConfig) (proffer.StageResult, error) {
	if err := a.validate(); err != nil {
		return proffer.StageResult{}, err
	}
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("open %s members: %w", cfg.stage, err)
	}
	defer stream.Close()
	writer, err := a.Repository.BeginHashBatch(ctx, BatchSpec{
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("begin %s batch: %w", cfg.stage, err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	for {
		...
		if nextErr != nil {
			return proffer.StageResult{}, fmt.Errorf("read %s member: %w", cfg.stage, nextErr)
		}
		if member.Reader == nil || member.SubjectRef == "" {
			return proffer.StageResult{}, fmt.Errorf("%s member %d lacks reader or subject reference", cfg.stage, count)
		}
		if member.Ordinal != count {
			_ = member.Reader.Close()
			return proffer.StageResult{}, fmt.Errorf("%s member ordinal %d, want %d", cfg.stage, member.Ordinal, count)
		}
		canon, canonErr := cfg.canon(member)
		if canonErr != nil {
			_ = member.Reader.Close()
			return proffer.StageResult{}, canonErr
		}
		...
		if hashErr != nil {
			return proffer.StageResult{}, fmt.Errorf("hash %s member %q: %w", cfg.stage, member.SubjectRef, hashErr)
		}
		if closeErr != nil {
			return proffer.StageResult{}, fmt.Errorf("close %s member %q: %w", cfg.stage, member.SubjectRef, closeErr)
		}
		if err := writer.Append(ctx, HashMember{SubjectRef: member.SubjectRef, Ordinal: count, Digest: digest, Canon: canon}); err != nil {
			return proffer.StageResult{}, fmt.Errorf("persist %s member %q: %w", cfg.stage, member.SubjectRef, err)
		}
		count++
		a.heartbeat(ctx, Progress{Stage: cfg.stage, MembersComplete: count, BytesComplete: progress.total})
	}
	if count == 0 {
		return proffer.StageResult{}, fmt.Errorf("%s refuses to seal an empty member manifest", cfg.stage)
	}
	resultRef, receiptRef, err := writer.Commit(ctx, HashSummary{Canon: cfg.manifestCanon, MemberCount: count})
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("commit %s: %w", cfg.stage, err)
	}
	committed = true
...
func (a HashActivities) FingerprintRawGeneration(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
...
func (a HashActivities) LegacyHashRawGeneration(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	if req.Refs["raw_fingerprint_manifest"] == "" && req.Refs["raw_hash_manifest"] != "" {
		refs := make(map[string]proffer.Ref, len(req.Refs)+1)
...
func (a HashActivities) HashNormalizedGeneration(ctx context.Context, req proffer.StageRequest) (proffer.StageResult, error) {
	return a.hashDigestGeneration(ctx, req, generationHashConfig{
...
func (a HashActivities) hashDigestGeneration(ctx context.Context, req proffer.StageRequest, cfg generationHashConfig) (proffer.StageResult, error) {
	if err := a.validate(); err != nil {
		return proffer.StageResult{}, err
	}
	...
	if err != nil {
		return proffer.StageResult{}, fmt.Errorf("open %s digest members: %w", cfg.stage, err)
	}
	defer stream.Close()
	writer, err := a.Repository.BeginHashBatch(ctx, BatchSpec{
	...
	for {
		...
		if nextErr != nil {
			return proffer.StageResult{}, fmt.Errorf("read %s digest member: %w", cfg.stage, nextErr)
		}
		if member.Ordinal != count {
			return proffer.StageResult{}, fmt.Errorf("%s digest ordinal %d, want %d", cfg.stage, member.Ordinal, count)
		}
		if !cfg.acceptCanon(member.Canon) {
			return proffer.StageResult{}, fmt.Errorf("%s member %q has incompatible canon %q", cfg.stage, member.SubjectRef, member.Canon)
		}
		if err := validateDigest(member.Digest); err != nil {
			return proffer.StageResult{}, fmt.Errorf("%s member %q: %w", cfg.stage, member.SubjectRef, err)
		}
		if err := acc.Add(member); err != nil {
			return proffer.StageResult{}, fmt.Errorf("fold %s member %q: %w", cfg.stage, member.SubjectRef, err)
		}
		if err := writer.Append(ctx, HashMember{SubjectRef: member.SubjectRef, Ordinal: count, Digest: member.Digest, Canon: member.Canon}); err != nil {
			return proffer.StageResult{}, fmt.Errorf("persist %s membership %q: %w", cfg.stage, member.SubjectRef, err)
		}
		count++
		a.heartbeat(ctx, Progress{Stage: cfg.stage, MembersComplete: count})
	}
	if count == 0 {
		return proffer.StageResult{}, fmt.Errorf("%s refuses to seal an empty ordered membership", cfg.stage)
	}
	digest := acc.Sum()
	resultRef, receiptRef, err := writer.Commit(ctx, HashSummary{
...
func requiredRef(req proffer.StageRequest, name string) (proffer.Ref, error) {
	ref := req.Refs[name]
	if ref == "" {
...
func success(stage stagegraph.StageID, resultRef, receiptRef proffer.Ref) proffer.StageResult {
