...
Copied verbatim (paths only) from the donor kit's services/api/tests/test_structure.py
— this is the one piece of the donor test suite that survives, per the build
spec ("copy the kit's structural/boundary tests"). The layer order and
forbidden-import rules are unchanged; only the SDK boundary list still names
donor SDKs (langchain, sentence_transformers) that this app doesn't use —
harmless to keep, since the assertion is "must not appear outside repo/".
"""
