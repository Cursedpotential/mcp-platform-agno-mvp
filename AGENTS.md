...
| `modules/vestigia/` (**vestigia**, D-140; the rename from `modules/traceIQ/` lands with the directory-rename step, old name kept as a junction) | **Nested independent product repo, gitignored** (contains its own nested `traceiq-rebuild` repo). | its own `AGENTS.md` |
