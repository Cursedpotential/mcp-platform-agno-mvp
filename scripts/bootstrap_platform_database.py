...
pieces sql/0036_context_import_foundation.sql is expected to build on: the `platform_admin` and
`platform_runtime` role names, the minimal extension set, and a `public.schema_version` ledger
table — nothing more. It never drops, renames, or otherwise mutates `ai` or any other database,
and it never mutates anything at all unless `--apply` is passed (default: dry-run/plan only).

sql/0036_context_import_foundation.sql and the platform-DB apply/validator scripts belong to a
different work lane and had not landed in this checkout as of authoring (2026-08-27). Rather than
guess their contents, this script detects sql/0036 live (see `discover_required_extensions`) and
degrades to its own minimal, clearly-documented default when the file is absent — see
sql/bootstrap/platform_foundation.sql's header for the full rationale.
...
def build_plan(target_database: str, state: LiveState, extension_gap: Sequence[str]) -> list[PlanStep]:
    ...
    steps = [
        ...
        PlanStep(f"create role {PLATFORM_RUNTIME_ROLE!r} (NOLOGIN)", already_satisfied=state.runtime_role_exists),
        PlanStep(
...
def apply_bootstrap(settings: ConnectionSettings, target_database: str, pgbin: pathlib.Path, checksum: str) -> None:
    ...
    runtime_role_sql = (
        "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = "
        f"'{PLATFORM_RUNTIME_ROLE}') THEN CREATE ROLE {PLATFORM_RUNTIME_ROLE} NOLOGIN; END IF; END $$;"
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(
        f"\napplied. {PLATFORM_ADMIN_ROLE}/{PLATFORM_RUNTIME_ROLE} verified, {args.target_db!r} "
