...
def _dense_and_text(text_desc: str) -> list[FieldSpec]:
    out = [
        FieldSpec("text", "varchar", max_length=_TEXT_MAX,
                  enable_analyzer=ENABLE_SPARSE_BM25, desc=text_desc),
...
def to_pymilvus_schema(spec: CollectionSpec):
    ...
    _DT = {
        "int64": DataType.INT64, "varchar": DataType.VARCHAR, "bool": DataType.BOOL,
        "double": DataType.DOUBLE, "json": DataType.JSON,
        "float_vector": DataType.FLOAT_VECTOR, "sparse_vector": DataType.SPARSE_FLOAT_VECTOR,
    }
    fields = []
    ...
    functions = [
        Function(name=fn.name, function_type=FunctionType.BM25,
                 input_field_names=[fn.input_field], output_field_names=[fn.output_field])
        for fn in spec.functions
    ]
    return CollectionSchema(
        fields=fields, description=spec.description,
        enable_dynamic_field=spec.enable_dynamic_field, functions=functions or None,
    )
...
def create_forensic_collections(client=None, dry_run: bool = True) -> dict[str, Any]:
    ...
    plan = {
        ...
        "collections": [
            {"name": s.name, "fields": [f.name for f in s.fields],
             "vector_dim": EMBED_TEXT_DIM, "functions": [fn.name for fn in s.functions]}
            for s in specs
        ],
